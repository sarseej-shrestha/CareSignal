...
- **SDOH transportation card** — a triggered suggestion (not a booking flow) pointing to real, individually researched local transportation resources (Councils on Aging, American Cancer Society Road To Recovery, Medicaid NEMT, LA 211); surfaces only for patients on a frequent (weekly/every-2-weeks) treatment schedule, since that's when transportation becomes a recurring barrier — see `lib/transportationResources.ts`.
