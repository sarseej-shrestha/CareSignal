...
- **SDOH transportation card** — a triggered suggestion (not a booking flow) pointing to real, researched local transportation resources, gated on treatment frequency rather than parish alone — see `lib/transportationResources.ts`.
- **FHIR-lite export** — downloads a patient's data as a FHIR R4 bundle, validated clean against HL7's real reference validator — see `docs/fhir-validation-results.md` for the full results.
