...
- **The dashboard says this itself, visibly, without being asked** — a small "What CareSignal does not claim" panel sits right below the header, open by default (not buried in a settings page): not clinically validated, not a diagnostic tool, not a replacement for clinician judgment, trained on simulated data, requires clinical workflow integration before real deployment. If a judge is scanning for whether this team overclaims, the answer is on screen before they even ask.
...
- The challenge brief explicitly names caregivers as a population the solution should serve — this isn't a bolted-on feature, it's a first-class second data source with its own alert type (`CAREGIVER_BURDEN`).
- **The dashboard's priority queue is a unified alert card, not two lists to cross-reference.** Each patient's card pairs their clinical status badge with their caregiver-burden badge side by side when flagged — Ruth Trahan's row literally reads "YELLOW" next to "Caregiver burden," in one place. The detail panel carries the same pairing further: a "Why this risk level" box sits directly beside a "Why caregiver burden is flagged" box, same tier, same view. This is a **presentation** change only — the two scores are still computed completely separately and never blended into one number; pairing them visually is exactly what makes it obvious, in the first three seconds of looking at the queue, that this tool tracks something every competitor's tool doesn't.
...
## RED safety bounce-back — "what happens right now if it's really bad"

A real gap closed, worth mentioning proactively if a judge doesn't ask: the auto-reply on a RED-level report used to be fairly soft ("a nurse will call you shortly... call your clinic or 911"). It now leads with the direct instruction: *"This requires prompt medical attention. Please call [clinic line] now, or call 911 if this feels like an emergency. A member of your care team has also been notified."*

- **Deterministic, not LLM-dependent** — this fires from `riskAckMessage()` immediately after the rules engine (Layer 1) sets RED, whether the input came in structured or as freeform text the AI parsed. Once symptom values are known, from either path, the RED call and this reply are 100% rules-engine output — there's no further LLM step a timeout or error could skip. Verified with dedicated tests covering both paths plus a caregiver relaying a RED report on the patient's behalf.
- **Language-aware**, like every other reply — fires in English, French, or Spanish depending on the sender's preference.
- **The clinic phone number is a real configuration point, not a hardcoded fake one** — set via `CLINIC_TRIAGE_PHONE` in `.env`. Left unset (as it is in this hackathon build, since we don't have a real triage line to publish), the message uses an obviously-fake `[clinic phone not configured]` placeholder rather than a plausible-looking number nobody authorized this app to hand out. Say this if asked: "We didn't want to demo a made-up phone number that looks real — this is exactly the kind of thing that needs one real config value before a real deployment, and the app makes that obvious rather than silently faking it."
...
- **On the dashboard, this panel is deliberately positioned and styled as secondary** — dashed border, smaller type, labeled "Prototype estimate," placed after the clinical risk reasons and the feature-demo cards (SDOH/SOAP/FHIR/trend chart) in the detail panel, not right after the header. If asked why: the daily clinical risk (rules-based, interpretable hard-stop thresholds) is the most defensible number this app produces and should get first attention; this forecast is a genuinely useful second signal but is trained on simulated data and not clinically validated, so it shouldn't visually compete with the clinical risk for a viewer's first read. Still fully present and functional — just not first.
...
- **Provenance you can check in seconds, not just a draft banner:** every note carries a "Sourced from these check-ins" list underneath the S/O/A/P text — the exact check-ins (date, Patient SMS/Caregiver SMS, and the raw pain/nausea/fatigue/fever numbers) that were actually fed into that generation, captured at generation time so it can't drift from what the model saw. If the note says "pain rose to 5/10," a nurse can look one line down and see the real number, not just trust the prose. Deliberately not a fake per-sentence citation — the whole note is generated from this same complete window at once, so this shows the real, complete evidence set rather than claiming a precision the generation process doesn't have.

One-line answer if asked directly: "Every note is a draft by default, at the database level, not just in the UI — and the review status travels with the text wherever it's copied. The model doesn't get to say how confident it is; we compute that separately from how much real data it had to work with. And every claim in the note sits right above the actual check-in data it came from, so a nurse can sanity-check it in a few seconds."
...
- **FHIR-lite export:** downloads a patient's data as FHIR-shaped resources (Patient, Condition, Observation, RiskAssessment, Flag). If asked "is this really FHIR": "It uses real FHIR resource types and real, individually verified LOINC codes where a matching one exists — we checked, we didn't guess. Where no matching code exists (there's no standard 0-10 nausea-severity LOINC code), we say so explicitly and use a local code instead of making one up. And we didn't just assume it's valid — we ran it through HL7's own reference validator (the public HAPI FHIR server) four times as we found and fixed real structural bugs (missing fullUrls, bad reference formats, an invalid Bundle.total field). The current version comes back with zero errors on every patient we've checked — genuinely FHIR-conformant, not just FHIR-shaped. The only remaining issues are cosmetic best-practice recommendations (missing narrative text, missing performer, our local nausea code being unregistered), and how many of those show up just scales with how much data that patient has — around 90-95 for a typical patient, 111 for our most data-rich seeded case (Ruth Trahan), just 3 for a brand-new patient with no check-ins yet. Full round-by-round results are in `docs/fhir-validation-results.md`. It's not a certified EHR integration — that's a separate formal process — but it's real, verified conformance, not a guess."
...
- **Found and fixed a real calibration mistake while building this**, worth mentioning if asked how confident to be in the number: the hospitalization-alert threshold was first set by eyeballing the seeded demo patients' scores (0.35 looked reasonable against those 8). Checked against the full simulated population instead, and that threshold fired on ~60% of all patient-days — useless as an "elevated" signal. Fixed by pinning it to the model's own trained decision boundary instead of a hand-picked number. Say this if asked "how do you know your alert threshold is right": "We checked it against more than the demo data, and the first number we picked was wrong by a lot — worth knowing before deployment, not after."

## If asked about testing / robustness

- 137 automated tests (Vitest, 16 files) — unit tests for every risk-engine rule boundary, both trained models, the AI parsing (mocked), and integration tests hitting the actual webhook/SOAP/FHIR logic against a real test database, including a real (not mocked) Twilio signature computation.
